...
Each login opens a temporary HTTP listener at `http://127.0.0.1:<port>/callback` on an available loopback port, as required by Devin. This works independently of the main server's TLS settings and bind address. The listener closes when login succeeds, fails, is cancelled, or expires. When the browser can reach that loopback listener (local deployment or an appropriate tunnel), it accepts the redirect automatically. Remote deployments can use step 3 without exposing the callback port. This is a browser authorization-code flow, not a device-code flow.
